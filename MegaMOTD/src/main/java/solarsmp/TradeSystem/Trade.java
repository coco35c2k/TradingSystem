...
public class Trade {
    ...
    public static Inventory GenerateTradeInventory(Player Trader, String OtherTraderName)
    {
        ...
        ItemStack red = TradeSystem.getConfirmRedOwn(Trader);
        for(int i = 0; i < 9; ++i)
        {
            inv.setItem(i, EmptyStack);
            inv.setItem(i + 36, red);
            if(i == 4) red = TradeSystem.getConfirmRedOther(Trader);;
        }
        for(int i = 0; i < 6; ++i)
        {
    ...
    {
        Bukkit.getScheduler().runTaskLater(TradeSystem.main,
                () -> {
    ...
    private void Confirmed()
    {
        ...
        ConfirmStage = 0;
        ConfirmScheduler = Bukkit.getScheduler().scheduleSyncRepeatingTask(TradeSystem.main, () -> {
            ...
            TradeInventoryPlayer1.setItem(45 + ConfirmStage, TradeSystem.getConfirmLoadingBar(Bukkit.getPlayer(uuidPlayer1)));
            TradeInventoryPlayer2.setItem(45 + ConfirmStage, TradeSystem.getConfirmLoadingBar(Bukkit.getPlayer(uuidPlayer2)));
    ...
    public void ToggleTradeConfirm(UUID Player)
    {
        ...
        {
            own = TradeSystem.getConfirmRedOwn(Player);
            other = TradeSystem.getConfirmRedOther(Other);
        }
        else
        {
            own = TradeSystem.getConfirmGreenOwn(Player);
            other = TradeSystem.getConfirmGreenOther(Other);
        }
